...
// Kiralama Onayı - VERİTABANINA KAYDET (Güncellenmiş Versiyon)
function confirmRentalModal() {
    ...
    let userEmailInput = document.getElementById('modal-user-email').value.trim();
    
    // TELEFON DÜZELTMESİ: Maskeli (parantezli) inputtan sadece rakamları çekiyoruz
    // Örn: "(0551) 444-33-22" -> "05514443322"
    let userPhoneRaw = document.getElementById('modal-user-phone').value.replace(/\D/g, '');
    ...
    // 2. Email kontrolü ve formatlama
    if (!userEmailInput) {
        alert('Lütfen e-mail adresinizi giriniz.');
        return;
    }
    
    // Eğer kullanıcı emailin sonuna manuel @gmail.com vb. eklemediyse butonlardan gelen veya otomatik tamamlama
    // (Not: Zaten input event ile hallediyoruz ama garantiye alalım)
    let finalEmail = userEmailInput;
    if (!finalEmail.includes('@')) {
        finalEmail += '@gmail.com';
    }
    
    // 3. Telefon kontrolü (11 hane: 05XXXXXXXXX)
    if (userPhoneRaw.length !== 11) {
        alert('Lütfen telefon numaranızı eksiksiz giriniz.');
        return;
    }
    
    // 4. İsimin ilk harflerini büyük yap (Görsel tutarlılık için)
    const nameParts = userName.split(' ').filter(part => part.length > 0);
    const capitalizedName = nameParts.map(part => 
        part.charAt(0).toUpperCase() + part.slice(1).toLowerCase()
    ).join(' ');

    // VERİTABANINA KAYDET (DCL kullanıcısı ile Stored Procedure çağrısı)
    fetch(`${API_URL}/kiralamalar/baslat`, {
        ...
        body: JSON.stringify({ 
            ...
            userPhone: userPhoneRaw // Saf rakam dizisi gönderiliyor
        })
    })
    .then(res => {
    ...
    .then(data => {
        console.log('Kiralama başarıyla başlatıldı:', data);
        ...
        // Başarı ekranı (Success Modal) bilgilerini doldur
        const now = new Date();
        ...
        document.getElementById('success-user-email').textContent = finalEmail;
        document.getElementById('success-user-phone').textContent = userPhoneRaw;
        document.getElementById('success-start-time').textContent = now.toLocaleString('tr-TR');
        
        // Başarı modal'ını göster
        $('#successModal').modal('show');
        
        // Verileri anlık yenile (Trigger ve View sonuçlarını görmek için)
        setTimeout(() => {
    ...
    .catch(err => {
        ...
        alert('Kiralama başlatılamadı! Detay: ' + err.message);
    });
}
